...
(define (asm-run pc vm)
  ...
  (define (call* next pc)
    (let ((n (+ (vm-nargs vm) (length (vm-closure-vars closure)))))
      ...
      (log/pp `(call put args ,(vm-closure-vars closure) nargs: ,(vm-nargs vm)
                sp: ,(vm-sp vm)))
      ;(log/pp `(regs-before: ,(vm-regs-frame vm)))
      (vm-put-args (vm-closure-vars closure) (+ (vm-sp vm) (vm-nargs vm)) vm)
      ;(log/pp `(regs-after: ,(vm-regs-frame vm)))
      (set-vm-nargs! vm n)
  ...
  (define (tail-fn-> fn nargs)
    (set! closure fn)
    (set! tmp-reg-nargs (vm-regs-ref vm (asm-nargs-reg current-fn vm)))
        ...
        (let ((x (vector-ref (asm-closure-code pc0) pc1)))
          ...
          (case (car x)
            ((klvm.load-reg->)
             ...
             (loop pc0 (+ pc1 1)))
            ((klvm.load-ret->)
             (set-vm-sp! vm (- (vm-sp vm)
                               (asm-closure-frame-size current-fn)))
            ...
            ((klvm.closure-tail-reg->)
             (tail-fn-> (vm-regs-ref vm (cadr x)) (caddr x))
             (loop pc0 (+ pc1 1)))
            ((klvm.closure-tail-fn->)
             (tail-fn-> (vm-fn-ref vm (cadr x)) (caddr x))
             (loop pc0 (+ pc1 1)))
            ((klvm.closure-tail-lambda->)
             (tail-fn-> (table-ref (vm-closures vm) (cadr x)) (caddr x))
             (loop pc0 (+ pc1 1)))
