...
(define (parse-asm code vm)
  ...
  (define (mk-func expr)
    (apply (lambda (name arity nregs nregs+ . body)
             (mk-vm-closure name arity (fn nregs nregs+ body) '()))
           (cdr expr)))
  (let loop ((code code)
             (toplevel '()))
    (if (pair? code)
...
(define (read-asm-from-file file vm)
  ...
  (define (read-data)
    (let loop ((acc '())
               (body '()))
      (let ((op (read-op (read-good-line (current-input-port)))))
        (cond ((not (pair? op)) (reverse (prepend body acc)))
              ((member (car op)
                       '(klvm.s1.func klvm.s1.closure klvm.s1.toplevel))
               (loop (prepend body acc) (reverse op)))
              (#t (loop acc (cons op body)))))))
  '(with-input-from-file file
    (lambda ()
      (let loop ()
        (let ((line (read-good-line)))
          (if (eof-object? line)
              )))))
  (parse-asm (with-input-from-file file read-data) vm))
...
(define (asm-run-code pp pi vm)
  ...
  (define (loop)
    ...
    (case (car op)
      ...
      ((closure-fn->) (closure-> (vm-fn-ref* vm (cadr op)) (caddr op)))
      ((closure-lambda->)
