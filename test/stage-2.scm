...
(define (put-closure-args offset closure vm)
  (let ((nargs (length (vm-closure-vars closure))))
    ...
    (vm-put-args (vm-closure-vars closure) (+ (vm-sp vm) offset) vm)
    (set-vm-nargs! vm (+ (vm-nargs vm) nargs))))
...
(define (klvm-eval func label vm)
  ...
  (define (eval-2 expr)
    ...
    (if (pair? expr)
        (case (car expr)
          ((klvm.reg) (vm-regs-ref vm (cadr expr)))
          ((klvm.ret) (vm-ret vm))
          ...
          (case (car expr)
            ...
            ((klvm.closure->)
             (with-exception-catcher
               (lambda (e) (mk-vm-error e "unknown function"))
               (lambda ()
            ...
            (else
             (case (car expr)
               ...
               ((klvm.put-closure-args)
                (put-closure-args (vm-nargs vm) closure vm))
               ((klvm.ret->) (set-vm-ret! vm (eval-2 (cadr expr))))
               ((klvm.nregs->) (vm-nregs-> vm (eval-2 (cadr expr))))
               ((klvm.reg->)
                (vm-regs-set! vm (cadr expr) (eval-2 (caddr expr))))
               ((klvm.next->)
...
(define (test.klvm.setup)
  (reset-vm! *klvm*)
  (read-test-defs)
  (read-test-klvm2 *klvm*))
...
(define (test.klvm)
  (clear-log)
  (test.klvm.setup)
  (test klvm-expr (table-ref test-defs current-test)))

(define (test.klvm-n n)
  (klvm.t-x (test-ref (table-ref test-defs current-test) n)))
